...
Why Not to Trigger Modals from Modals or Dropdowns

## Summary

Avoid mixing dropdowns, search boxes, and modals in a single interaction. Each pattern has its place, but combining them creates accessibility failures, cognitive overload, and usability friction. Instead, use recognized patterns (combo box, explicit advanced search links, or inline search fields) that set clear expectations and maintain accessible focus flows.

## Scenarios

### Triggering a Modal from Another Modal
...
Better Alternatives
	•	Use inline disclosure (accordion, expandable section).
	•	Use a single modal with progressive steps (wizard/stepper).
...
### Triggering a Modal from a Dropdown
...
Usability Issues
	...
	•	Error recovery: Once the modal closes, the dropdown is gone — context is lost.
...
### Search Box Inside a Dropdown That Opens a Modal
